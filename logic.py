    ...
    for i in range(c.GRID_LEN):
        ...
        for j in range(c.GRID_LEN):
            if mat[i][j] != 0:
                ...
                count_stop = count
        tiles_moved += count - count_stop
    return new, done, tiles_moved
...
def up(game):
    ...
    game = transpose(game)
    game, done, tiles_moved = cover_up(game)
    game, done, score = merge(game, done)
    game = cover_up(game)[0]
    game = transpose(game)
    return game, done, score, tiles_moved
